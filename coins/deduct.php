<?php
require_once __DIR__ . '/../helpers.php';

if ($_SERVER['REQUEST_METHOD'] !== 'POST') err('Método no permitido', 405);

$payload = auth();
$b       = body();
$coins  = (int)($b['coins'] ?? 0);
$reason = $b['reason'] ?? '';
if (!$coins) err('Datos inválidos');

$pdo = db();
$stmt = $pdo->prepare('SELECT coins FROM users WHERE id = ?');
$stmt->execute([$payload['uid']]);
$current = (int)$stmt->fetchColumn();
if ($current < $coins) err('Monedas insuficientes');

$pdo->prepare('UPDATE users SET coins = coins - ? WHERE id = ?')->execute([$coins, $payload['uid']]);
$pdo->prepare('INSERT INTO coin_transactions (id, user_id, coins, reason, type) VALUES (?,?,?,?,?)')
    ->execute([uuid(), $payload['uid'], $coins, $reason, 'deduction']);

$stmt->execute([$payload['uid']]);
ok(['coins' => (int)$stmt->fetchColumn()]);
